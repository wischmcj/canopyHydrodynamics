...
#     # flexible_collection.draw('XZ', highlight_lambda = lambda: branch_order ==1, highlight = True)
...
def test_find_flows_small_tree(flexible_collection):
    ...
    flexible_collection.initialize_graph()
    flexible_collection.find_flow_components()
    flexible_collection.calculate_flows(plane="XY")
    flexible_collection.statistics(plane="XY")
    # actual = flexible_collection.flows
    # flexible_collection.draw(highlight_lambda=lambda : is_stem, filter_lambda = lambda:cyl_id>100 )
    # expected = small_tree_flows
    # assert expected == str(actual)
