...
def unpickle_collection(
    file_name: str, is_full_path: bool = False
) -> CylinderCollection:
    """
...
class CylinderCollection:
    ...
    def __eq__(self, other):
        ...
        Returns:
            bool: True if the CylinderCollections are equal, False otherwise.
    ...
    def from_csv(self, file, aggregate_cyls=True):
        """
        Initializes a new Cylinder Collection based on the data in a QSM with the configured column locations.

        Args:
            file: The file object or file name to read the data from.
            aggregate_cyls (bool): If True, aggregates values from the file. Default is True.
    ...
    def draw(
        ...
        **kwargs,
        ...
        Args:
            plane (str): The plane to draw the cylinders on. Valid values are "XY", "XZ", "YZ", and "3D".
            highlight_lambda (Callable): A lambda function that determines which cylinders to highlight.
            filter_lambda (Callable): A lambda function that determines which cylinders to include in the drawing.
            include_drips (bool): If True, includes drip points in the drawing. Default is False.
            include_contour (bool): If True, includes contour lines in the drawing. Default is False.
            include_alpha_shape (bool): If True, includes alpha shapes in the drawing. Default is False.
            stem (bool): If True, includes the stem in the drawing. Default is False.
            file_name_ext (str): The file extension to use for the output file. Default is an empty string.
        ...
        # if as_vectors:
        #     to_draw=[[cyl.x,cyl.y,cyl.z] for cyl in self.filtered_cyls]
        #     draw_vectors( to_draw,
        #                  colors=matches,
        ...
        if plane == "3D":
            ...
            if as_vectors:
                ...
                to_draw = [
                    cyl.projected_data[plane]["polygon"] for cyl in filtered_cyls
                ]
    ...
    def get_end_nodes(self) -> list[int]:
        ...
        If the end nodes have already been computed, it returns the cached result.
    ...
    def get_trunk_nodes(self) -> list[int]:
        ...
        If the trunk nodes have already been computed, it returns the cached result.
        Otherwise, it computes the trunk nodes based on the branch order of the cylinders.

        Returns:
            A list of trunk nodes in the digraph.
        """
        g = self.digraph
        if self.trunk_nodes:
    ...
    def watershed_boundary(
        ...
        Args:
            ...
            stem: If True, the input component is filtered by 'is_stem'=True. Defaults to False.
